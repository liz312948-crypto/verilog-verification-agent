...
- A final 107-test suite covering unit, real-Icarus integration, end-to-end repair, retry
  exhaustion, packaging, and cross-platform path validation.

### Changed

- Normalized the Python distribution version to PEP 440 `0.1.0a1` while retaining the
  `v0.1.0-alpha` release label.
...
- Stopped persisting raw rejected model responses in run evidence; reports retain only their
  SHA-256 digests.
- Applied both POSIX and Windows lexical path semantics on every host, rejecting absolute,
  drive-qualified, rooted, UNC, and parent-traversal paths consistently.
...
- All 107 tests passed on Windows locally and in Ubuntu GitHub Actions CI. Publication is gated
  on the Ubuntu CI result and occurs only after that verification passes.
